...
export const SupplementAdherenceChart: React.FC<SupplementAdherenceChartProps> = ({
  data,
  className = "",
}) => {
  ...
  const chartData: SupplementAdherenceData[] = useMemo(() => {
    ...
    return [
      { period: 'Week 1', creatine: 95, protein: 88, vitamins: 75, preWorkout: 60, betaAlanine: 85, performance: 82 },
      { period: 'Week 2', creatine: 90, protein: 92, vitamins: 80, preWorkout: 65, betaAlanine: 88, performance: 85 },
      { period: 'Week 3', creatine: 85, protein: 85, vitamins: 90, preWorkout: 70, betaAlanine: 90, performance: 78 },
      { period: 'Week 4', creatine: 100, protein: 95, vitamins: 85, preWorkout: 80, betaAlanine: 92, performance: 92 },
      { period: 'Week 5', creatine: 95, protein: 90, vitamins: 88, preWorkout: 85, betaAlanine: 95, performance: 89 },
      { period: 'Week 6', creatine: 98, protein: 93, vitamins: 92, preWorkout: 88, betaAlanine: 98, performance: 94 },
    ];
  }, [data]);
  ...
  const metrics = useMemo(() => {
    ...
    const supplementaryAdherence = (latest.vitamins + latest.preWorkout + latest.betaAlanine) / 3;
    const overallAdherence = (latest.creatine + latest.protein + latest.vitamins + latest.preWorkout + latest.betaAlanine) / 5;
    
    const performanceCorrelation = chartData.reduce((sum, week) => {
      const weeklyAdherence = (week.creatine + week.protein + week.vitamins + week.preWorkout + week.betaAlanine) / 5;
      return sum + (weeklyAdherence * week.performance / 10000); // Simple correlation
    }, 0) / chartData.length;
    
    const trend = latest.performance > previous.performance ? 'improving' : 
                  latest.performance < previous.performance ? 'declining' : 'stable';
  ...
  const aiInsights: AIInsight[] = useMemo(() => {
    ...
    const latest = chartData[chartData.length - 1];
    const lowestAdherence = Math.min(latest.vitamins, latest.preWorkout, latest.betaAlanine);
    if (lowestAdherence < 75) {
      const lowestSupplement = latest.vitamins === lowestAdherence ? 'vitamins' :
                              latest.preWorkout === lowestAdherence ? 'pre-workout' : 'beta-alanine';
      
      insights.push({
        type: 'recommendation',
        title: `${lowestSupplement.charAt(0).toUpperCase() + lowestSupplement.slice(1)} Focus Needed`,
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Track supplement consistency and correlation with performance metrics
              </CardDescription>
            </div>
            <div className="flex gap-2">
              <Badge variant="outline" className={`${metrics.trend === 'improving' ? 'bg-green-50 text-green-700 border-green-200' : metrics.trend === 'declining' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-gray-50 text-gray-700 border-gray-200'}`}>
                {metrics.trend === 'improving' ? '↗' : metrics.trend === 'declining' ? '↘' : '→'} {metrics.trend}
              </Badge>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-purple-50 p-3 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-gray-700">Core Adherence</span>
              </div>
              <div className="text-xl font-bold text-purple-600">{metrics.coreAdherence}%</div>
            ...
            <div className="bg-blue-50 p-3 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-gray-700">Support Stack</span>
              </div>
              <div className="text-xl font-bold text-blue-600">{metrics.supplementaryAdherence}%</div>
            ...
            <div className="bg-green-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-green-600">{metrics.performanceScore}</div>
            ...
            <div className="bg-orange-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-orange-600">{metrics.performanceCorrelation}</div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <Line 
                  ...
                  strokeWidth={3}
                  dot={{ fill: CHART_COLORS.warningOrange, strokeWidth: 2, r: 4 }}
          ...
          <div className="mt-6 bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white p-3 rounded border border-purple-200">
                <div className="text-sm text-gray-600 mb-1">Adherence-Performance Correlation</div>
                <div className="text-lg font-bold text-purple-600">{metrics.performanceCorrelation}</div>
                <div className="text-xs text-gray-500">Strong correlation = better results</div>
              </div>
              <div className="bg-white p-3 rounded border border-blue-200">
                <div className="text-sm text-gray-600 mb-1">Improvement Potential</div>
                <div className="text-lg font-bold text-blue-600">{metrics.improvementPotential}%</div>
                <div className="text-xs text-gray-500">Room for performance gains</div>
              </div>
              <div className="bg-white p-3 rounded border border-green-200">
                <div className="text-sm text-gray-600 mb-1">Consistency Rating</div>
                <div className="text-lg font-bold text-green-600">
                  {metrics.overallAdherence > 90 ? 'Excellent' : 
                   metrics.overallAdherence > 80 ? 'Good' : 
                   metrics.overallAdherence > 70 ? 'Fair' : 'Needs Work'}
                </div>
                <div className="text-xs text-gray-500">Based on overall adherence</div>
              </div>
            </div>
          </div>
        </CardContent>
