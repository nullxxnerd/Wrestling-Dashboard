...
import { ScatterChart, Scatter, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, ComposedChart, Bar } from 'recharts';
...
export const PerformanceCorrelationChart: React.FC<PerformanceCorrelationChartProps> = ({
  data,
  className = "",
}) => {
  ...
  const chartData: PerformanceDataPoint[] = useMemo(() => {
    ...
    return Array.from({ length: 12 }, (_, i) => {
      ...
      const strengthScore = Math.min(100, (60 + adherenceEffect * 30 + Math.random() * 15));
      const enduranceScore = Math.min(100, (55 + adherenceEffect * 35 + Math.random() * 15));
      const powerOutput = Math.min(100, (50 + adherenceEffect * 40 + Math.random() * 15));
      const recoveryScore = Math.min(100, (65 + adherenceEffect * 25 + Math.random() * 15));
      const competitionPerformance = Math.min(100, (60 + adherenceEffect * 30 + Math.random() * 20));
  ...
  const correlationMetrics = useMemo(() => {
    ...
    const strengthCorrelation = calculateCorrelation(adherenceValues, strengthValues);
    const enduranceCorrelation = calculateCorrelation(adherenceValues, enduranceValues);
    const powerCorrelation = calculateCorrelation(adherenceValues, powerValues);
    const recoveryCorrelation = calculateCorrelation(adherenceValues, recoveryValues);
    
    const avgPerformance = chartData.reduce((sum, d) => 
      sum + (d.strengthScore + d.enduranceScore + d.powerOutput + d.recoveryScore) / 4, 0) / chartData.length;
    ...
    return {
      ...
      strongestCorrelation: Math.max(strengthCorrelation, enduranceCorrelation, powerCorrelation, recoveryCorrelation),
    };
  }, [chartData]);
  ...
  const scatterData = chartData.map(d => ({
    ...
    performance: (d.strengthScore + d.enduranceScore + d.powerOutput + d.recoveryScore) / 4,
    week: d.date,
  ...
  const aiInsights: AIInsight[] = useMemo(() => {
    ...
    const recentAvg = recentPerformance.reduce((sum, d) => sum + d.competitionPerformance, 0) / recentPerformance.length;
    const earlierAvg = earlierPerformance.reduce((sum, d) => sum + d.competitionPerformance, 0) / earlierPerformance.length;
    
    if (recentAvg > earlierAvg + 5) {
      insights.push({
        type: 'optimization',
        title: 'Performance Trending Up! 📈',
        content: `Recent performance improved by ${Math.round(recentAvg - earlierAvg)}% compared to earlier weeks. Your supplement strategy is working.`,
        priority: 'medium',
        actionable: false,
      });
    }
    ...
    const avgCreatine = chartData.reduce((sum, d) => sum + d.creatineIntake, 0) / chartData.length;
    const avgProtein = chartData.reduce((sum, d) => sum + d.proteinIntake, 0) / chartData.length;
    
    if (avgCreatine < 4 && correlationMetrics.powerCorrelation > 0.6) {
      insights.push({
        type: 'recommendation',
        title: 'Creatine Optimization Opportunity',
        content: `Strong power correlation suggests increasing creatine from ${Math.round(avgCreatine * 10) / 10}g to 5g daily could boost performance further.`,
        priority: 'medium',
        actionable: true,
    ...
    if (avgProtein < 170 && correlationMetrics.recoveryCorrelation > 0.6) {
      insights.push({
        ...
        content: `Strong recovery correlation indicates increasing protein from ${Math.round(avgProtein)}g to 180-200g could improve recovery metrics.`,
        priority: 'medium',
        actionable: true,
    ...
    const bestWeek = chartData.reduce((prev, current) => 
      current.competitionPerformance > prev.competitionPerformance ? current : prev);
        ...
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <div className="space-y-1 text-sm">
            ...
            {(data.performance || data.value) && (
              <p>Performance: {Math.round((data.performance || data.value) as number)}</p>
            )}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Track the relationship between supplement intake and athletic performance
              </CardDescription>
            </div>
            <div className="flex gap-2">
              ...
              <Badge variant="outline" className={`${correlationMetrics.avgPerformance > 80 ? 'bg-green-50 text-green-700 border-green-200' : correlationMetrics.avgPerformance > 60 ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-blue-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-blue-600">r = {correlationMetrics.strengthCorrelation}</div>
            ...
            <div className="bg-green-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-green-600">r = {correlationMetrics.enduranceCorrelation}</div>
              <div className="text-xs text-gray-600">Adherence correlation</div>
            </div>
            ...
            <div className="bg-purple-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-purple-600">r = {correlationMetrics.powerCorrelation}</div>
              <div className="text-xs text-gray-600">Adherence correlation</div>
            </div>
            ...
            <div className="bg-orange-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-orange-600">r = {correlationMetrics.recoveryCorrelation}</div>
              <div className="text-xs text-gray-600">Adherence correlation</div>
            </div>
          </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart data={scatterData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <XAxis 
                      ...
                      tick={{ fill: '#6b7280' }}
                      domain={[60, 100]}
                      label={{ value: 'Adherence %', position: 'insideBottom', offset: -5 }}
                    />
                    <YAxis 
                      type="number" 
                      dataKey="performance" 
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="strengthScore" fill={CHART_COLORS.primaryBlue} opacity={0.7} />
                    <Bar dataKey="enduranceScore" fill={CHART_COLORS.successGreen} opacity={0.7} />
                    <Bar dataKey="powerOutput" fill={CHART_COLORS.dangerRed} opacity={0.7} />
                    <Bar dataKey="recoveryScore" fill={CHART_COLORS.warningOrange} opacity={0.7} />
                    ...
                    <Line 
                      ...
                      strokeWidth={3}
                      dot={{ fill: CHART_COLORS.purpleAccent, strokeWidth: 2, r: 4 }}
                    />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="h-40">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  />
                  <Tooltip 
                    formatter={(value: number) => [`${value}%`, 'Competition Performance']}
                  ...
                  <Line 
                    ...
                    activeDot={{ r: 7, stroke: CHART_COLORS.dangerRed, strokeWidth: 2 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-blue-900">Strongest Link</span>
              </div>
              <div className="text-lg font-bold text-blue-600">
                {correlationMetrics.powerCorrelation === correlationMetrics.strongestCorrelation ? 'Power Output' :
                 correlationMetrics.enduranceCorrelation === correlationMetrics.strongestCorrelation ? 'Endurance' :
                 correlationMetrics.strengthCorrelation === correlationMetrics.strongestCorrelation ? 'Strength' : 'Recovery'}
              </div>
              <div className="text-xs text-blue-700">r = {correlationMetrics.strongestCorrelation}</div>
            </div>
            ...
            <div className="bg-green-50 p-3 rounded-lg border border-green-200">
              ...
              <div className="text-lg font-bold text-green-600">
                {correlationMetrics.avgPerformance > 80 ? 'Excellent' : 
                 correlationMetrics.avgPerformance > 65 ? 'Good' : 'Improving'}
              </div>
              <div className="text-xs text-green-700">{correlationMetrics.avgPerformance}% average</div>
            </div>
            ...
            <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
              ...
              <div className="text-lg font-bold text-purple-600">
                {correlationMetrics.strongestCorrelation > 0.7 ? 'Optimized' :
                 correlationMetrics.strongestCorrelation > 0.5 ? 'Good' : 'Needs Work'}
              </div>
              <div className="text-xs text-purple-700">Based on correlations</div>
            </div>
          </div>
        </CardContent>
      </Card>
